...
app.post('/api/guardar', (req, res) => {
	...
	const params = [[usuario, nombre, password]]
	var connection = mysql.createConnection(credentials)
	connection.query('INSERT INTO usuarios (usuario, nombre, password) VALUES ?', [params], (err, result) => {
		if (err) {
			res.status(500).send(err)
		} else {
...
app.post('/api/editar', (req, res) => {
	...
	var connection = mysql.createConnection(credentials)
	connection.query('UPDATE usuarios set usuario = ?, nombre = ?, password = ? WHERE id = ?', params, (err, result) => {
		if (err) {
			res.status(500).send(err)
		} else {
...
app.listen(4000, async () => {
	...
	console.log(figlet.textSync('ReservatuMesa v. 1.0.0'))
})